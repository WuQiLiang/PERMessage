...
using PERMessage.Filters;
using PERMessage.Models;
using PERMessage.Service;

namespace PERMessage.Controllers
{
    public class AccountController : Controller
    {
        private PERMessageEntities db = new  PERMessageEntities();
        //
        // GET: /Account/Login
        ...
        public ActionResult Login(PERMessage.Models.LoginModel model)
        {
            ...
            per_User User = null;
            UserLoginStatus result = manager.userLogin(username, password, out User);
            if (result == UserLoginStatus.Success)
            {
                //将验证查询到的登录对象 放置在Session中
                Session["user"] = User; //key--value
                return RedirectToAction("Index", "Home");
            }
            else
            {
                if (result == UserLoginStatus.IsPwdError)
                {
                    ViewBag.error = true;
                    ViewBag.message = "密码错误!";
                    return View(model);
                }
                if (result == UserLoginStatus.IsDisabled)
                {
                    ViewBag.error = true;
                    ViewBag.message = "账户被锁定!";
                    return View(model);
                }
            }
